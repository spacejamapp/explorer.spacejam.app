...
export default async function EpochPage({
  ...
  const cores = epoch.cores.nodes.map(
    (core: {
      id: number;
      gasUsed: number;
      imports: number;
  ...
  const vals_current = epoch.validators.nodes.map(
    (val: {
      id: number;
      blocks: number;
      tickets: number;
  ...
  const totalBlocks = vals_current.reduce(
    (sum: number, val: { blocks: number }) => sum + val.blocks,
    0
  );
  const totalGasUsed = cores.reduce(
    (sum: number, core: { gas_used: number }) => sum + core.gas_used,
    0
  );
  const totalExtrinsics = cores.reduce(
    (sum: number, core: { extrinsic_count: number }) =>
      sum + core.extrinsic_count,
    0
  );
  const totalBundleSize = cores.reduce(
    (sum: number, core: { bundle_size: number }) => sum + core.bundle_size,
    0
  );
